...
cmd /c "tailscale ssh root@pds-laptop-old ""cd /var/lib/lifeos-host-daemon && rm -rf web && mkdir -p web && tar -xf - -C web"" < $tmp\web.tar"

# Deploy Linux daemon binary
Write-Host "5. Uploading lifeos-linux daemon binary..." -ForegroundColor Yellow
tar -cf "$tmp\bin.tar" -C "$tmp" lifeos-linux
cmd /c "tailscale ssh root@pds-laptop-old ""cd /var/lib/lifeos-host-daemon && tar -xf - && chmod +x lifeos-linux && (systemctl restart lifeos-host-daemon 2>/dev/null || systemctl restart lifeos 2>/dev/null || true)"" < $tmp\bin.tar"
...
if (Test-Path "$tmp\idx.html") {
    ...
    if ($remoteHash -eq $localHash) {
        ...
        Write-Host " NOTE: Web index hash differs. Check cache or reload browser." -ForegroundColor Yellow
    }
}

Remove-Item "$tmp\web.tar", "$tmp\bin.tar", "$tmp\lifeos-linux", "$tmp\server_linux" -Force -ErrorAction SilentlyContinue
